...
\setmainfont{Times New Roman}[Kerning=Off]%
% [ Path = ../EBGaramond/,
%   Extension = .otf,
%   UprightFont = *-Regular,
%   BoldFont = *-bold,
...
% \setmathfont{TeX Gyre Termes Math}
\setmathfont{AuroraMath.otf}[Path=../Release/]%[StylisticSet={2,8,9,6},Path=./]
% \setmathfont{Garamond-Mathx.otf}%[StylisticSet={2,8,9,6},Path=./]
...
\NewDocumentCommand \SUPSUBMAP { m m }
  { \[ \exp_args:Nx \tl_map_inline:nn {#2} {{ #1{##1 \sb{x}\sp{x}} }} \] }
...
\ExplSyntaxOff
\def\OVERUNDERLINE#1{%
  #1{} \quad #1{b} \quad #1{ab} \quad #1{abj} \quad #1{abcd} \quad #1{abcde} \quad #1{a+b+c}}
\def\LISTTEXT{A_1, \, B_2, \, x_3,\ x_4\, \ \ldots, \, x_n}
...
$:;\mathcolon\epsilon\symup\epsilon$
\[\begin{split}
...
\[a_{ij}({{}j})ij TijT{ijT_oT_x^xL_{ff}L_{jj}Uo}iUojj_x^xJjflTlfxTx[f][j][y]\sin\cos\tan\exp\symup{WTV}(\symsf {fijk})\int_0^u\]
...
\[ \symbffrak{\Latinalphabets \latinalphabets} \]
...
%\[{\Uhextensible width 1cm 0 "2192}{\Uhextensible width 1cm 0 "21DB}{\Uhextensible width 1cm 0 "21CE}\]
  \item Math accents
        ...
        \[\widetilde y \tilde y \tilde z \hat U \widehat{U} \hat \psi \widehat{\psi}\]
  ...
  \item Large operators (inline)
        $ \int\limits_0^\infty \int_0^\infty \int_0^1 \iint \dd{x} \iiint \dd{y}
...
&{}=\frac{1}{\sqrt{2^n}}\bigotimes_{l=1}^{n}\bigotimes_{+}^{+}\left[\ket{0}_{n-l}+\ee^{-2\uppi \ii ({0.j_{l-1}\ldots j_0})}\ket{1}_{n-l}\right].
\end{split}
\]
